...
std::double_t ObaraSakia::Kinetic::computePrimitive1D(std::double_t centerA, std::double_t exponentA, std::int64_t shellA, std::double_t centerB, std::double_t exponentB, std::int64_t shellB, std::double_t gaussianCenter, std::double_t gaussianExponent)
{
    if ((shellA < 0) || (shellB < 0))
    {
        return 0.0;
    ...
    else if ((shellA == 0) && (shellB == 0))
    {
        ...
        std::double_t T20 = exponentA + (T10 * -2 * pow(exponentA, 2));
        return S00 * T20;
    }
    ...
    else if (shellB == 0 && shellA > 0)
    {
        return (
            ...
            (1 / (2 * (exponentA + exponentB))) * ((shellA - 1) * ObaraSakia::Kinetic::computePrimitive1D(centerA, exponentA, shellA - 2, centerB, exponentB, shellB, gaussianCenter, gaussianExponent) +
                                                   (shellB + 0) * ObaraSakia::Kinetic::computePrimitive1D(centerA, exponentA, shellA - 1, centerB, exponentB, shellB - 1, gaussianCenter, gaussianExponent)) +
            (exponentB / (exponentA + exponentB)) * (2 * exponentA * ObaraSakia::Overlap::computePrimitive1D(centerA, exponentA, shellA, centerB, exponentB, shellB, gaussianCenter, gaussianExponent) -
                                                     (shellA - 1) * ObaraSakia::Overlap::computePrimitive1D(centerA, exponentA, shellA - 2, centerB, exponentB, shellB, gaussianCenter, gaussianExponent))
        );
    }
    else
    {
        return (
            (gaussianCenter - centerB) * ObaraSakia::Kinetic::computePrimitive1D(centerA, exponentA, shellA, centerB, exponentB, shellB - 1, gaussianCenter, gaussianExponent) +
            (1 / (2 * (exponentA + exponentB))) * ((shellA + 0) * ObaraSakia::Kinetic::computePrimitive1D(centerA, exponentA, shellA - 1, centerB, exponentB, shellB - 1, gaussianCenter, gaussianExponent) +
                                                   (shellB - 1) * ObaraSakia::Kinetic::computePrimitive1D(centerA, exponentA, shellA, centerB, exponentB, shellB - 2, gaussianCenter, gaussianExponent)) +
            (exponentA / (exponentA + exponentB)) * (2 * exponentB * ObaraSakia::Overlap::computePrimitive1D(centerA, exponentA, shellA, centerB, exponentB, shellB, gaussianCenter, gaussianExponent) -
                                                     (shellB - 1) * ObaraSakia::Overlap::computePrimitive1D(centerA, exponentA, shellA, centerB, exponentB, shellB - 2, gaussianCenter, gaussianExponent))
        );
    }
}

std::double_t ObaraSakia::Kinetic::computePrimitive3D(
    cxx_Primitive primitiveA, std::double_t xA, std::double_t yA, std::double_t zA, std::int64_t lxA, std::int64_t lyA, std::int64_t lzA,
    cxx_Primitive primitiveB, std::double_t xB, std::double_t yB, std::double_t zB, std::int64_t lxB, std::int64_t lyB, std::int64_t lzB,
    std::double_t gaussianCenterX, std::double_t gaussianCenterY, std::double_t gaussianCenterZ, std::double_t gaussianExponent)
{
    std::double_t TxDir = ObaraSakia::Kinetic::computePrimitive1D(xA, primitiveA.primitive_exp, lxA, xB, primitiveB.primitive_exp, lxB, gaussianCenterX, gaussianExponent);
    std::double_t TyDir = ObaraSakia::Kinetic::computePrimitive1D(yA, primitiveA.primitive_exp, lyA, yB, primitiveB.primitive_exp, lyB, gaussianCenterY, gaussianExponent);
    std::double_t TzDir = ObaraSakia::Kinetic::computePrimitive1D(zA, primitiveA.primitive_exp, lzA, zB, primitiveB.primitive_exp, lzB, gaussianCenterZ, gaussianExponent);

    std::double_t SxDir = ObaraSakia::Overlap::computePrimitive1D(xA, primitiveA.primitive_exp, lxA, xB, primitiveB.primitive_exp, lxB, gaussianCenterX, gaussianExponent);
    std::double_t SyDir = ObaraSakia::Overlap::computePrimitive1D(yA, primitiveA.primitive_exp, lyA, yB, primitiveB.primitive_exp, lyB, gaussianCenterY, gaussianExponent);
    std::double_t SzDir = ObaraSakia::Overlap::computePrimitive1D(zA, primitiveA.primitive_exp, lzA, zB, primitiveB.primitive_exp, lzB, gaussianCenterZ, gaussianExponent);

    return ((TxDir * SyDir * SzDir) + (SxDir * TyDir * SzDir) + (SxDir * SyDir * TzDir)) * pow(M_PI / (primitiveA.primitive_exp + primitiveB.primitive_exp), 1.5);
    // return (xDir + yDir + zDir) * pow(M_PI / (primitiveA.primitive_exp + primitiveB.primitive_exp), 1.5);
}

std::double_t ObaraSakia::Kinetic::computeContracted(cxx_Contracted contractedGaussianA, cxx_Contracted contractedGaussianB)
{
    // data for first shell
    std::double_t xA = contractedGaussianA.location_x;
    std::double_t yA = contractedGaussianA.location_y;
    ...
    for (std::uint64_t ii = 0; ii < nA; ii++)
    {
        for (std::uint64_t jj = 0; jj < nB; jj++)
        {
            ...
            std::double_t value = ObaraSakia::Kinetic::computePrimitive3D(
                contractedGaussianA.contracted_GTO[ii], xA, yA, zA, lxA, lyA, lzA,
                contractedGaussianB.contracted_GTO[jj], xB, yB, zB, lxB, lyB, lzB,
                productGaussianAB.gaussian_center[0], productGaussianAB.gaussian_center[1], productGaussianAB.gaussian_center[2], productGaussianAB.gaussian_exponent);
