...
void IntegralEngine::computeOverlap(cxx_Calculator &planckCalculator, Eigen::MatrixXd &overlapMatrix)
{
    ...
    for (std::uint64_t row = 0; row < nBasis; row++)
    {
        ...
        {
            std::double_t value = ObaraSakia::Overlap::computeContracted(planckCalculator.calculation_set[row], planckCalculator.calculation_set[col]);
...
void IntegralEngine::computeKinetic(cxx_Calculator &planckCalculator, Eigen::MatrixXd &kineticMatrix)
{
    ...
    for (std::uint64_t row = 0; row < nBasis; row++)
    {
        ...
        {
            std::double_t value = ObaraSakia::Kinetic::computeContracted(planckCalculator.calculation_set[row], planckCalculator.calculation_set[col]);
